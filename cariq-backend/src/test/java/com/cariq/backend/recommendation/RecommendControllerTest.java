...
class RecommendControllerTest {
    ...
    void recommend_validRequest_returnsShortlistWithReasoningAndTradeoff() throws Exception {
        // "10-20 lakhs" → [10L, 20L] → min=1,000,000 max=2,000,000
        // nexon (10.5L ✓), creta (18.5L ✓), nexonEv (19.95L ✓) — swift (6.65L) is BELOW 10L, excluded
        ...
        when(carRepository.findById(2L)).thenReturn(Optional.of(creta));
        stubOpenAi("{\"shortlist\":[\"3\",\"2\"],\"reasoning\":{\"3\":\"Great city mileage\",\"2\":\"Top safety score\"},\"tradeoff\":\"Nexon is affordable but Creta is safer\"}");
        ...
        mockMvc.perform(post("/api/recommend")
                ...
                .andExpect(jsonPath("$.reasoning['3']", is("Great city mileage")))
                .andExpect(jsonPath("$.reasoning['2']", is("Top safety score")))
                .andExpect(jsonPath("$.tradeoff", containsString("affordable")));
    }
    ...
    void recommend_budgetRange_usesUpperBoundAsFilter() throws Exception {
        // "5-15 lakhs" → [5L, 15L] → min=500,000 max=1,500,000
        // swift (6.65L ✓) and nexon (10.5L ✓) qualify; creta (18.5L) and nexonEv (19.95L) are above max
        when(carRepository.findAll()).thenReturn(List.of(swift, creta, nexon, nexonEv));
        when(carRepository.findById(1L)).thenReturn(Optional.of(swift));
        when(carRepository.findById(3L)).thenReturn(Optional.of(nexon));
        ...
        mockMvc.perform(post("/api/recommend")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req("5-15 lakhs", "city/family", "NCAP", "safety"))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.shortlist", hasSize(2)));
    }
    ...
    private void stubOpenAi(String content) {
        when(openAiClient.complete(anyString(), anyString())).thenReturn(content);
    }
